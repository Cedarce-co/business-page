...
function SectionBlock({
    ...
    return (
      <section className={`${altBg} py-16 lg:py-20`}>
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-black text-cliq-text-heading lg:text-3xl">{section.title}</h2>
  ...
  if (section.type === "bento") {
    return (
      <section className={`${altBg} py-16 lg:py-20`}>
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-black text-cliq-text-heading lg:text-3xl">{section.title}</h2>
          {section.subtitle ? <p className="mt-3 max-w-2xl text-cliq-text-body">{section.subtitle}</p> : null}
      ...
      <section className={`${altBg} py-16 lg:py-20`}>
        <div className="mx-auto grid max-w-[1200px] gap-10 px-4 sm:px-6 lg:grid-cols-2 lg:items-center lg:px-8">
          <div className={section.reverse ? "lg:order-2" : ""}>
            <h2 className="text-2xl font-black text-cliq-text-heading lg:text-3xl">{section.title}</h2>
            <p className="mt-4 text-cliq-text-body leading-relaxed">{section.body}</p>
          </div>
          <div className={`rounded-2xl border p-6 lg:p-8 ${a.panel} ${section.reverse ? "lg:order-1" : ""}`}>
          ...
          <ol
            className={
              section.orientation === "horizontal"
                ? "mt-10 grid gap-8 lg:grid-cols-3"
                : "relative mt-10 space-y-8 border-l-2 border-cliq-gray-200 pl-8"
            }
          >
  ...
  if (section.type === "checklist") {
    return (
      <section className={`${altBg} py-16 lg:py-20`}>
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-black text-cliq-text-heading lg:text-3xl">{section.title}</h2>
          <ul className={`mt-8 grid gap-3 ${section.columns === 2 ? "sm:grid-cols-2" : ""}`}>
            {section.items.map((item) => (
  ...
  if (section.type === "comparison") {
    return (
      <section className={`${altBg} py-16 lg:py-20`}>
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8">
          <h2 className="text-center text-2xl font-black text-cliq-text-heading lg:text-3xl">{section.title}</h2>
...
function Hero({ page }: { page: MarketingPageConfig }) {
  ...
  const backHref = page.category === "product" ? "/about" : "/services";
  const backLabel = page.category === "product" ? "Product" : "Solutions";

  if (page.heroVariant === "split") {
    return (
      <section className="border-b border-cliq-gray-200 bg-[linear-gradient(165deg,#f8fafc_0%,#ffffff_50%)]">
        <div className="mx-auto max-w-[1200px] px-4 py-12 sm:px-6 lg:grid lg:grid-cols-2 lg:items-center lg:gap-12 lg:px-8 lg:py-20">
          <div>
            <Link href={backHref} className="inline-flex items-center gap-1 text-sm font-semibold text-cliq-text-muted hover:text-cliq-navy-900">
            ...
            <h1 className="mt-4 text-4xl font-black text-cliq-text-heading lg:text-5xl">{page.title}</h1>
            <p className="mt-2 text-xl font-semibold text-cliq-purple">{page.tagline}</p>
            <p className="mt-4 text-cliq-text-body leading-relaxed">{page.lead}</p>
          ...
          <div className={`rounded-3xl bg-gradient-to-br p-8 text-white ${a.heroBg}`}>
      ...
      <section className="border-b border-cliq-gray-200 bg-white py-16 lg:py-24">
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 max-w-3xl">
            ...
            <p className="mt-4 text-xl text-cliq-purple">{page.tagline}</p>
            <p className="mt-4 text-lg leading-relaxed text-cliq-text-body">{page.lead}</p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Button href={page.primaryCta.href} variant="teal" className="rounded-xl px-6 py-3 text-sm font-bold">
                {page.primaryCta.label}
              </Button>
  ...
  const centered = page.heroVariant === "centered";
  return (
    <section className={`relative overflow-hidden bg-gradient-to-b pb-20 pt-12 ${a.heroBg}`}>
      <div className="relative mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8">
        <Link href={backHref} className="inline-flex items-center gap-1 text-sm font-semibold text-white/70 hover:text-white">
          <ArrowLeft className="h-4 w-4" aria-hidden />
          {backLabel}
        </Link>
        <div className={centered ? "mx-auto mt-8 max-w-3xl text-center" : "mt-8 max-w-3xl"}>
          <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/10">
            <Icon className="h-8 w-8 text-cliq-teal" aria-hidden />
          </span>
          <p className={`mt-6 text-sm font-semibold uppercase tracking-wider text-cliq-teal ${centered ? "mx-auto" : ""}`}>
            {page.eyebrow}
          </p>
          <h1 className="mt-3 text-4xl font-black text-white lg:text-5xl">{page.title}</h1>
          <p className="mt-3 text-xl font-semibold text-white/90">{page.tagline}</p>
          <p className="mt-4 text-base leading-relaxed text-white/75">{page.lead}</p>
  ...
  return (
    <>
      ...
      <section className="border-t border-cliq-gray-200 bg-cliq-gray-100 py-16">
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8">
          <h2 className="text-xl font-black text-cliq-text-heading">Related {page.category === "product" ? "products" : "solutions"}</h2>
